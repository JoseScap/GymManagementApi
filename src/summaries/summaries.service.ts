...
export class SummariesService {
    ...
    async getLastDay() {
        ...
        const date = lastSubs[0].createdAt
    ...
    async signLastDay() {
        const data = await this.getLastDay();

        const summary = this.summaryRepository.create({
            day: getDate(data.date),
            week: getWeek(data.date, { weekStartsOn: WEEK_STARTS_ON }),
            month: getMonth(data.date) + 1,
